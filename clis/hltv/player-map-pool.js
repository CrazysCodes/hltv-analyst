...
function finish(target) {
  ...
  return {
    ...
    maps: target.maps,
    avgRating: target.ratingCount ? round(target.ratingSum / target.ratingCount) : null,
    kdRatio: target.deaths > 0 ? round(target.kills / target.deaths) : null,
    killsPerMap: target.maps ? round(target.kills / target.maps) : null,
    plusMinusTotal: target.plusMinusTotal,
    winMaps: target.wins,
    lossMaps: target.losses,
    details: {
      winRatePct: target.maps ? round((target.wins / target.maps) * 100) : null,
      deathsPerMap: target.maps ? round(target.deaths / target.maps) : null,
